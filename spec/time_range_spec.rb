...
RSpec.describe TimeRange do
  ...
  describe '.split_by_calendar_day' do
    ...
    describe 'returned array' do
      it 'has cardinality equal to number of calendar days in the time range' do
        expect(partials.count).to eq(calendar_days_in(time_range))
      end

      describe 'first time range partial' do
        subject(:first) { partials.first }

        it 'starts same as the time range' do
      ...
      describe 'middle time range partial' do
      ...
      describe 'last time range partial' do
        subject(:last) { partials.last }

        it 'starts at the beginning of the last calendar day' do
          expect(last.begin).to eq(time_range.end.beginning_of_day)
        end

        it 'ends same as the time range' do
